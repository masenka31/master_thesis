...
dataset = ARGS[2]
# dataset = "animals"
checkpath = datadir("experiments", "MIProblems", dataset, "classifier_triplet", "seed=1")
mkpath(checkpath)
...
function sample_params()
    ...
    α = sample([0.1f0, 0.5f0, 1f0])
    return (hdim = hdim, ldim = ldim, batchsize = batchsize, agg = agg, activation = activation, margin=margin, α=α)
end
...
function experiment(dataset::String, pvec, seed, ratios, max_train_time)
    ...
    function loss(x, yoh, y)
        ...
        enc = model[1:end-1](x)
        trl = ClusterLosses.loss(Triplet(pvec.margin), SqEuclidean(), enc, y)
        return ce + pvec.α * trl
    end
    ...
    # @show nm
    safesave(datadir("experiments", "MIProblems", dataset, "classifier_triplet", "seed=$seed", nm), results)
    @info "Results saved."
end
