...
function sample_params()
    ...
    margin = sample([0.5f0, 1f0, 10f0])
    α = sample([0.1f0, 1f0, 10f0])
    return hdim, ldim, batchsize, agg, margin, α
end
...
# model parameters
hdim, ldim, batchsize, agg, margin, α = sample_params()
parameters = (hdim = hdim, ldim = ldim, batchsize = batchsize, agg = agg, margin = margin, α = α)
