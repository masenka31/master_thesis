...
ratios = (r, 0.5-r, 0.5)
checkpath = datadir("experiments", "gvma", "classifier_triplet", "seed=1")
mkpath(checkpath)
...
function sample_params()
    ...
    α = sample([0.1f0, 1f0])
    return (hdim = hdim, batchsize = batchsize, agg = agg, activation = activation, margin=margin, α=α)
end
...
function experiment(X, y, p, seed, ratios, r, max_train_time)
    ...
    function loss(x, yoh, y)
        ...
        enc = mill_model(x)
        trl = ClusterLosses.loss(Triplet(p.margin), SqEuclidean(), enc, y)
    ...
    # @show nm
    safesave(datadir("experiments", "gvma", "classifier_triplet", "seed=$seed", nm), results)
    @info "Results saved."
end
